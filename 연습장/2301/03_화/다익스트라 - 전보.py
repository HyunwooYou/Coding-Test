"""
* Title: 다익스트라 - 전보
어떤 나라에는 N개의 도시가 있다. 그리고 각 도시는 보내조가 하는 메세지가 있는 경우,
...
각 도시는 번호와 통로가 설치되어 있는 정보가 주어졌을 때, 도시 C에서 보낸 메세지를 받게 되는 도시의 개수는 총 몇 개이며
도시들이 모두 메세지를 받는 데까지 걸리는 시간은 얼마인지 계싼하는 프로그램을 작성하시오.
